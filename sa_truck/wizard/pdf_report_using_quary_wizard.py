...
class PdfReportQuaryWizard(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, driver):
        ...
        quary = """select create_datetime from sa_truck"""
        if start_date and end_date:
            driver1 = quary
        if driver1 >= start_date :
            self.env.cr.execute(quary)
        if driver1 <= end_date :
            self.env.cr.execute(quary)
        driver = self.env.cr.dictfetchall()
        ...
        #     # sheet.write(raction_sa_truck_xls_reportow, col, 'Create Date Time', bold)
